...
def load_resources():
    ...
    if not faiss_index_path.exists() or not meta_path.exists():
        raise RuntimeError(
            "Index files not found. Please run the indexing script first (e.g., `python -m src.semantic_search`)."
        )
...
def search(query: SearchQuery, db: sqlite3.Connection = Depends(get_db)):
    ...
    # them back into a single string for searching.
    try:
        if "ontology" in globals() and ontology:
            expanded_terms = expand_terms(query.query, ontology)
            expanded_query = " ".join(expanded_terms)
        else:
            # If no ontology is configured, use the original query.
            expanded_query = query.query
    except Exception:
        # In case of any error during expansion, fall back to the original query.
        expanded_query = query.query

    # Perform semantic search
    if query.mode in ["semantic", "hybrid"]:
        raw_sem_results = perform_semantic_search(
            faiss_index, expanded_query, model, chunks, refs, k=query.top_k * 5
        ...
        for ref, text, score in raw_sem_results:
            ...
            if doc_id.startswith("zotero-"):
                ...
                item_data = get_single_zotero_item(
                    conn, item_id, zotero_data_dir / "storage"
                )
                conn.close()
                ...
                if item_data:
                    ...
                    if query.tags and not any(
                        tag in item_data.get("tags", []) for tag in query.tags
                    ):
                        continue
                    if query.collections and not any(
                        c in item_data.get("collections", []) for c in query.collections
                    ):
                        continue
        ...
        for path, snippet in raw_key_results:
            ...
            if not any(r.path == path for r in semantic_results):
                keyword_results.append(
                    SearchResult(path=path, snippet=snippet, score=0.0)
                )  # Keyword score is not directly comparable
...
def get_document(doc_id: str):
    ...
    if doc_id.startswith("zotero-"):
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Invalid Zotero item ID format."
            )

        zotero_cfg = cfg.get("zotero", {})
        if not zotero_cfg.get("enabled"):
            raise HTTPException(
                status_code=404, detail="Zotero integration is not enabled in config."
            )
        ...
        if not conn:
            raise HTTPException(
                status_code=500, detail="Could not connect to Zotero database."
            )
        ...
        if not item_data:
            raise HTTPException(
                status_code=404, detail=f"Zotero item with ID {item_id} not found."
            )
        return item_data
    else:
        ...
        if Path(doc_id).exists():
            ...
            raise HTTPException(
                status_code=404, detail=f"Document with path '{doc_id}' not found."
            )
