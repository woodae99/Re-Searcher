# test_pipeline_notes_only.py - Pipeline test with Zotero notes (no PDF extraction)
# This shows a quicker test path - just notes and Obsidian
# Run from project root: python test_pipeline_notes_only.py [num_zotero] [num_obsidian]
...
def test_end_to_end_pipeline(num_docs_zotero=10, num_docs_obsidian=10):
    """
    Test the full pipeline with notes only (no PDF extraction).
    """
    print("=" * 70)
    print("END-TO-END PIPELINE TEST (ZOTERO NOTES + OBSIDIAN)")
    print("=" * 70)
    ...
    # Config for notes only (no PDF extraction)
    ...
    config["storage"]["collection_name"] = "test_notes_only"
    store = ChromaVectorStore(config)
    
    # Fetch documents from both sources
    documents = []
    
    print(f"\n[2/6] Fetching {num_docs_zotero} documents from Zotero (notes only)...")
    zotero_count = 0
    ...
    print(f"      ✓ Stored {count} chunks in collection 'test_notes_only' ({store_time:.2f}s)")
    ...
    for query in test_queries:
        ...
        for i, (doc, meta, dist) in enumerate(zip(
            ...
            title = meta.get("title", "Untitled")[:45]
    ...
    print("=" * 70)
    print(f"""
Performance Summary (Notes Only):
  ╭─────────────────────────────────────────────────────────────╮
  │ Sources                                                     │
  ├─────────────────────────────────────────────────────────────┤
  │ • Zotero notes:          {zotero_count:6d}  ({zotero_time:6.2f}s)           │
  │ • Obsidian docs:         {obsidian_count:6d}  ({obsidian_time:6.2f}s)           │
