# test_pipeline_with_attachments.py - Pipeline test with PDF extraction
# Run from project root: python test_pipeline_with_attachments.py [num_zotero] [num_obsidian]
...
def test_end_to_end_pipeline(num_docs_zotero=10, num_docs_obsidian=10):
    """
    Test the full pipeline with multiple sources and PDF extraction:
    1. Fetch documents from Zotero (with attachment extraction)
    2. Fetch documents from Obsidian
    3. Chunk the text
    ...
    """
    print("=" * 70)
    print("END-TO-END PIPELINE TEST (WITH PDF EXTRACTION)")
    print("=" * 70)
    ...
    # Enable attachment extraction for PDF processing
    ...
    config["storage"]["collection_name"] = "test_pipeline_attachments"
    ...
    print("=" * 70)
    print(f"""
Performance Summary (WITH PDF EXTRACTION):
  ╭─────────────────────────────────────────────────────────────╮
  │ Sources                                                     │
  ├─────────────────────────────────────────────────────────────┤
...
if __name__ == "__main__":
    ...
    # Usage: python test_pipeline_with_attachments.py [num_zotero] [num_obsidian]
    # Note: PDF extraction is slow, so recommend starting with 50-75 Zotero docs
    num_docs_zotero = int(sys.argv[1]) if len(sys.argv) > 1 else 50
    num_docs_obsidian = int(sys.argv[2]) if len(sys.argv) > 2 else 50
